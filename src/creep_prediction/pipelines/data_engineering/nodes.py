...
def _get_features(parameters: dict) -> list[str]:
    ...
    """
    features_always = ['Temperature', 'Load'] # These features are always used
    ...
    if isinstance(features, str): # Convert the string to a list if it is a string
        features = [features] 
    features = list(set(features_always + features)) # Combine the features and remove possible duplicates using set
...
def split_df(preprocessed_creep_data: pd.DataFrame, 
    ...
    model_data = df[features + [family_col] + [target]].dropna(axis=0)
    ...
    # Checking if indices match between info_data and model_data
    if not info_data.index.equals(model_data.index):
        raise ValueError("Indices of model_data and info_data do not match.")
    
    info_data = df[info_cols + list(set(features + R_cols + CM_cols))] # set to remove possible duplicates
    info_data = info_data.iloc[model_data.index] # keep only the rows that are in model_data
...
def feature_interactions(model_data: pd.DataFrame, 
    ...
    # For every feature, create a new column with its interaction with the features_always
    for feat_1 in features:
        for feat_always in features_always:
            model_data[f"*{feat_1}*{feat_always}"] = model_data[feat_1] * model_data[feat_always]
            new_features.append(f"{feat_1}*{feat_always}")
...
def feature_engineering(model_data: pd.DataFrame, 
    ...
    Args:
    -----
    - model_data (pd.DataFrame): The input DataFrame to be transformed.
